...
def remove_memory(usID, chID, film_name):
    ...
    dell = db_sess.query(Memory).filter(Memory.user_id == usID,
                                        Memory.chat_id == chID,
                                        Memory.film_id == film_name).first()
    if dell:
        db_sess.delete(dell)
        db_sess.commit()
    else:
...
def take_memory(usID, chID):
    db_sess = db_session.create_session()
    dell = db_sess.execute(select(Memory.film_id).where(Memory.user_id == usID,
...
def remove_notification(usID, chID, film_name):
    ...
    dell = db_sess.query(Notification).filter(Notification.user_id == usID,
                                              ...
                                              Notification.film_id == film_name).first()
    if dell:
        db_sess.delete(dell)
        db_sess.commit()
    else:
...
def take_notification(usID, chID):
    db_sess = db_session.create_session()
    dell = db_sess.execute(select(Notification.film_id).where(Notification.user_id == usID,
...
def top_5_users_film():
    db_sess = db_session.create_session()
    dell = db_sess.execute(select(Score.film_id, Score.score)).all()
    if dell:
        slov_film = {}
        for fl_name, sc in dell:  # пробегаемся по списку и заносим данные с словарь
            # sc = оценка фильма
...
def top_user_genre(usID, chID):
    ...
    if dell:
        slov_genre = {}
        for sc, gr in dell:  # пробегаемся по списку и заносим данные в словарь
            # sc = оценка фильма; gr = жанр
